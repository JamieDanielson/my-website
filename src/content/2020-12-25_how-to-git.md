...
When that's all set, go back to your terminal where you should still be in your branch. Type `git checkout master` to get back to the master branch. Then `git fetch` to get the changes from the remote master into your local master. Finally, `git branch -d my-name` will delete the local copy of the "my-name" branch. CONGRATULATIONS! You just created a repo, created a branch, made a commit, created a pull request, merged a branch into master, fetched changes, and deleted a branch.
